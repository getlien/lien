    ...
    for offset in range(0, len(sorted_recs), batch_size):
        ...
        for i, record in enumerate(batch):
            ...
            if matching_validation and matching_validation.success:
                ...
                for key, value in transformed.data.items():
                    ...
                    if isinstance(value, dict):
                        ...
                        for nk, nv in value.items():
                            nk_str = str(nk).lower().strip() if isinstance(nk, str) else str(nk).lower().strip()
                            nested[nk_str] = nv
                        data[lower_key] = nested
                    elif isinstance(value, list):
                        data[lower_key] = [
                            (
                                {str(ik).lower().strip(): iv for ik, iv in item.items()}
                ...
                all_results.append(
                    ProcessingResult(
                        record_id=record.id, success=True, errors=[], output=transformed.data
                    )
                )
