...
- ab10e5a: Export the call-site-level dependency graph (`buildDependencyGraph`, `isPreciseProvenance`, and the `DependencyGraph`/`SymbolNode`/`CallerEdge`/`EdgeProvenance`/`TransitiveCallerEdge`/`TransitiveResult`/`TransitiveOptions` types) from `@liendev/parser`.
...
- bddcdb9: Fix `matchesFile`'s Strategy 4 (`matchesPHPNamespace`) — a case-insensitive,
...
- cc4e511: Fix `mod dir;` fabricating dependency edges to every file under `dir/`,
