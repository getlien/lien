...
All four miss examples cite public PRs from the study record and are
independently checkable against the linked project's history, though only a
subset of the study's 24 fixtures is committed as replayable fixtures in the
repo's corpus.

## Reproducing this

The committed corpus lives at `packages/review/test/harness/fixtures/crossrepo/`
plus the per-rule canaries. Every fixture regenerates from its
`.assertions.ts` header's `capture-pr.ts` recipe against the public PR it was
mined from; fixtures tagged `characterization` (e.g. `werkzeug` #2678, #2017)
document known frontiers without gating anything. See
[How We Know Lien Review Works](/guide/review-harness) and
`packages/review/test/harness/README.md` for the calibration bar, the full
corpus table, and the harness workflow.
