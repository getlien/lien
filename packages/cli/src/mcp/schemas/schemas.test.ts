...
describe('FindSimilarSchema', () => {
  ...
  it('should reject short code snippet', () => {
    expect(() => FindSimilarSchema.parse({ code: 'short' })).toThrow(
      'Code snippet must be at least 24 characters',
    );
  });

  it('should reject invalid limit (too low)', () => {
    expect(() => FindSimilarSchema.parse({ code: 'const x = 1; return x + 2;', limit: 0 })).toThrow(
      'Limit must be at least 1',
    );
  });
  ...
  it('should reject invalid limit (too high)', () => {
    expect(() =>
      FindSimilarSchema.parse({ code: 'const x = 1; return x + 2;', limit: 25 }),
...
describe('GetFilesContextSchema', () => {
  ...
  it('should reject empty filepath', () => {
    expect(() => GetFilesContextSchema.parse({ filepaths: '' })).toThrow(
      'Filepath cannot be empty',
    );
  });

  it('should accept various filepath formats', () => {
    const paths = ['src/index.ts', './src/index.ts', 'packages/cli/src/index.ts', 'index.ts'];
...
describe('ListFunctionsSchema', () => {
  ...
  it('should accept various pattern formats', () => {
    const patterns = ['.*Service$', 'handle.*', '.*Controller.*', '^get', 'test'];
...
describe('GetDependentsSchema', () => {
  ...
  it('should reject empty filepath', () => {
    expect(() => GetDependentsSchema.parse({ filepath: '' })).toThrow('Filepath cannot be empty');
  });
...
describe('GetComplexitySchema', () => {
  ...
  it('should reject top > 50', () => {
    expect(() => GetComplexitySchema.parse({ top: 100 })).toThrow('Top cannot exceed 50');
  });

  it('should reject top < 1', () => {
    expect(() => GetComplexitySchema.parse({ top: 0 })).toThrow('Top must be at least 1');
  });

  it('should reject non-integer top', () => {
    expect(() => GetComplexitySchema.parse({ top: 5.5 })).toThrow();
  });

  it('should reject threshold < 1', () => {
    expect(() => GetComplexitySchema.parse({ threshold: 0 })).toThrow(
      'Threshold must be at least 1',
    );
  });

  it('should reject non-integer threshold', () => {
    expect(() => GetComplexitySchema.parse({ threshold: 10.5 })).toThrow();
  });

  it('should reject files array with empty strings', () => {
    expect(() => GetComplexitySchema.parse({ files: ['', 'src/auth.ts'] })).toThrow(
      'Filepath cannot be empty',
    );
  });
