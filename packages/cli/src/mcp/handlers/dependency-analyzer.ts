...
/**
 * Dependency analyzer for finding files that depend on a target file.
 */
export class DependencyAnalyzer {
  constructor(
    private vectorDB: VectorDBInterface,
    private log: (message: string, level?: 'warning') => void
  ) {}
  ...
  async findDependents(
    ...
    if (crossRepo && this.vectorDB instanceof QdrantDB) {
      allChunks = await this.vectorDB.scanCrossRepo({ limit: SCAN_LIMIT });
    } else {
      if (crossRepo) {
        this.log('Warning: crossRepo=true requires Qdrant backend. Falling back to single-repo search.', 'warning');
      }
      allChunks = await this.vectorDB.scanWithFilter({ limit: SCAN_LIMIT });
    }

    const hitLimit = allChunks.length === SCAN_LIMIT;
    if (hitLimit) {
      this.log(`Scanned ${SCAN_LIMIT} chunks (limit reached). Results may be incomplete.`, 'warning');
    }
    this.log(`Scanning ${allChunks.length} chunks for imports...`);
    ...
    const importIndex = this.buildImportIndex(allChunks, normalizePathCached);
    const normalizedTarget = normalizePathCached(filepath);
    const dependentChunks = this.findDependentChunks(importIndex, normalizedTarget);
    ...
    // Calculate metrics
    const fileComplexities = this.calculateFileComplexities(chunksByFile);
    const complexityMetrics = this.calculateOverallComplexityMetrics(fileComplexities);
   ...
   */
  private calculateFileComplexities(
    chunksByFile: Map<string, SearchResult[]>
  ): FileComplexity[] {
   ...
   */
  private calculateOverallComplexityMetrics(
    fileComplexities: FileComplexity[]
  ): ComplexityMetrics {
    ...
    const complexityRiskBoost = this.calculateComplexityRiskBoost(totalAvg, globalMax);
   ...
   */
  private calculateComplexityRiskBoost(avgComplexity: number, maxComplexity: number): RiskLevel {
