...
Ranking: results are ordered by BM25, then nudged by structural importance — files more other files depend on rank slightly higher among similarly-relevant matches. The nudge is capped and usually just breaks ties, but a very well-connected hub file can still outrank a marginally-better lexical match elsewhere. Each result's own score/relevance always describe its PURE lexical match quality (never recomputed from the nudge), so list order and an individual result's relevance label can legitimately disagree — trust the order for "what to look at first", trust relevance for "how good is this specific match". Set LIEN_STRUCTURAL_RANKING=off to disable and fall back to pure BM25 order.
