...
function pLimit(concurrency) {
  ...
  const enqueue = (function_, resolve, arguments_) => {
    queue.enqueue(
      import_async_hooks.AsyncResource.bind(run2.bind(void 0, function_, resolve, arguments_))
    );
    (async () => {
...
async function scanFramework(rootDir, framework) {
  const frameworkPath = import_path.default.join(rootDir, framework.path);
  const gitignorePath = import_path.default.join(frameworkPath, ".gitignore");
  ...
  try {
    ...
    const rootGitignorePath = import_path.default.join(rootDir, ".gitignore");
    try {
      const gitignoreContent = await import_promises2.default.readFile(rootGitignorePath, "utf-8");
      ig = (0, import_ignore2.default)().add(gitignoreContent);
    } catch (e2) {
    }
  }
  ...
  return uniqueFiles.filter((file) => !ig.ignores(file)).map((file) => {
    return framework.path === "." ? file : import_path.default.join(framework.path, file);
  });
}
async function scanCodebase(options) {
  ...
  return uniqueFiles.filter((file) => {
    const relativePath = import_path.default.relative(rootDir, file);
...
var languageConfig = {
  typescript: import_tree_sitter_typescript.default.typescript,
  javascript: import_tree_sitter_javascript.default,
  php: import_tree_sitter_php.default.php,
  // Note: tree-sitter-php exports both 'php' (mixed HTML/PHP) and 'php_only'
  python: import_tree_sitter_python.default
};
...
function detectLanguage2(filePath) {
  const ext2 = (0, import_path2.extname)(filePath).slice(1).toLowerCase();
  switch (ext2) {
    case "ts":
    case "tsx":
      return "typescript";
...
var LocalEmbeddings = class {
  ...
  async initialize() {
    ...
    this.initPromise = (async () => {
      try {
        this.extractor = await (0, import_transformers.pipeline)("feature-extraction", this.modelName);
      } catch (error2) {
...
async function writeVersionFile(indexPath) {
  try {
    const versionFilePath = import_path3.default.join(indexPath, VERSION_FILE);
    const timestamp = Date.now().toString();
    await import_promises3.default.writeFile(versionFilePath, timestamp, "utf-8");
  } catch (error2) {
...
async function readVersionFile(indexPath) {
  try {
    const versionFilePath = import_path3.default.join(indexPath, VERSION_FILE);
    const content = await import_promises3.default.readFile(versionFilePath, "utf-8");
...
function isDocumentationFile(filepath) {
  ...
  const filename = import_path4.default.basename(filepath).toLowerCase();
...
var FilenameBoostingStrategy = class {
  ...
  apply(query, filepath, baseScore) {
    const filename = import_path4.default.basename(filepath, import_path4.default.extname(filepath)).toLowerCase();
    const queryTokens = query.toLowerCase().split(/\s+/);
    let boostFactor = 1;
    for (const token of queryTokens) {
      if (token.length <= 2)
...
async function clear(db, table, tableName, dbPath) {
  ...
  try {
    if (dbPath) {
      const lanceDir = import_path5.default.join(dbPath, `${tableName}.lance`);
      try {
        await import_promises4.default.rm(lanceDir, { recursive: true, force: true });
      } catch (err) {
...
var VectorDB = class _VectorDB {
  ...
  constructor(projectRoot) {
    const projectName = import_path6.default.basename(projectRoot);
    const pathHash = import_crypto.default.createHash("md5").update(projectRoot).digest("hex").substring(0, 8);
    this.dbPath = import_path6.default.join(import_os.default.homedir(), ".lien", "indices", `${projectName}-${pathHash}`);
  }
  async initialize() {
    try {
    ...
    try {
      ...
      if (this.needsMigration(userConfig)) {
        ...
        if (result.migrated && result.backupPath) {
          const backupFilename = import_path7.default.basename(result.backupPath);
...
${validation.errors.join("\n")}`, { errors: validation.errors });
    ...
    try {
      ...
      await import_promises5.default.writeFile(configPath, configJson, "utf-8");
    } catch (error2) {
...
${validation.errors.join("\n")}`, { errors: validation.errors });
      ...
      await import_promises5.default.copyFile(configPath, backupPath);
  ...
  getConfigPath(rootDir) {
    return import_path7.default.join(rootDir, _ConfigService.CONFIG_FILENAME);
  }
  /**
  ...
  validateFrameworks(frameworks, errors, warnings) {
    ...
    frameworks.forEach((framework, index) => {
      ...
      } else if (import_path7.default.isAbsolute(fw.path)) {
...
var ManifestManager = class {
  ...
  constructor(indexPath) {
    ...
    this.manifestPath = import_path8.default.join(indexPath, MANIFEST_FILE);
  }
  /**
  ...
  async load() {
    try {
      const content = await import_promises6.default.readFile(this.manifestPath, "utf-8");
  ...
  async save(manifest) {
    try {
      await import_promises6.default.mkdir(this.indexPath, { recursive: true });
      ...
      await import_promises6.default.writeFile(this.manifestPath, content, "utf-8");
    } catch (error2) {
  ...
  async clear() {
    try {
      await import_promises6.default.unlink(this.manifestPath);
    } catch (error2) {
      if (error2.code !== "ENOENT") {
...
async function isGitRepo(rootDir) {
  try {
    const gitDir = import_path9.default.join(rootDir, ".git");
    await import_promises7.default.access(gitDir);
    return true;
  } catch {
    return false;
  }
...
async function getChangedFiles(rootDir, fromRef, toRef) {
  try {
    ...
    const files = stdout.trim().split("\n").filter(Boolean).map((file) => import_path9.default.join(rootDir, file));
    return files;
  } catch (error2) {
...
var GitStateTracker = class {
  ...
  constructor(rootDir, indexPath) {
    ...
    this.stateFile = import_path10.default.join(indexPath, ".git-state.json");
  }
  /**
  ...
  async loadState() {
    try {
      const content = await import_promises8.default.readFile(this.stateFile, "utf-8");
  ...
  async saveState(state) {
    try {
      ...
      await import_promises8.default.writeFile(this.stateFile, content, "utf-8");
    } catch (error2) {
...
function normalizeToRelativePath(filepath, rootDir) {
  ...
  if (!import_path11.default.isAbsolute(filepath)) {
    return normalized;
  }
  ...
  return import_path11.default.relative(root, filepath).replace(/\\/g, "/");
}
...
async function processSingleFileForIndexing(filepath, normalizedPath, embeddings, config, verbose) {
  try {
    const stats = await import_promises9.default.stat(filepath);
...
async function mtimeBasedDetection(rootDir, savedManifest, config) {
  ...
  for (const filepath of currentFiles) {
    try {
      const absolutePath = import_path12.default.isAbsolute(filepath) ? filepath : import_path12.default.join(rootDir, filepath);
      const stats = await import_promises10.default.stat(absolutePath);
...
async function processFileForIndexing(file, batchProcessor, indexConfig, progressTracker, _verbose) {
  try {
    const stats = await import_promises11.default.stat(file);
    const content = await import_promises11.default.readFile(file, "utf-8");
...
async function analyzeBaseBranch(baseSha, filesToAnalyze, threshold) {
  try {
    ...
    const currentHead = (0, import_child_process2.execSync)("git rev-parse HEAD", { encoding: "utf-8" }).trim();
    (0, import_child_process2.execSync)(`git checkout --force ${baseSha}`, { stdio: "pipe" });
    ...
    (0, import_child_process2.execSync)(`git checkout --force ${currentHead}`, { stdio: "pipe" });
