...
exports.formatComplexityValue = formatComplexityValue;
exports.formatThresholdValue = formatThresholdValue;
exports.buildReviewPrompt = buildReviewPrompt;
exports.buildNoViolationsMessage = buildNoViolationsMessage;
exports.formatReviewComment = formatReviewComment;
exports.getViolationKey = getViolationKey;
exports.buildDescriptionBadge = buildDescriptionBadge;
exports.buildLineCommentPrompt = buildLineCommentPrompt;
exports.buildLineSummaryComment = buildLineSummaryComment;
exports.buildBatchedCommentsPrompt = buildBatchedCommentsPrompt;
...
function formatComplexityValue(metricType, value) {
    switch (metricType) {
        case 'halstead_effort':
            return `~${(0, format_js_1.formatTime)(value)}`;
        case 'halstead_bugs':
...
function buildDeltaContext(deltas) {
    ...
    const formatChange = (d) => {
        ...
        return `  - ${d.symbolName}: ${from} → ${to} (${(0, delta_js_1.formatDelta)(d.delta)})`;
    };
    const sections = [
...
function categorizeDeltas(deltas) {
    return deltas.reduce((acc, d) => {
        if (['improved', 'deleted'].includes(d.severity))
            acc.improved++;
        else if (['warning', 'error', 'new'].includes(d.severity))
            acc.degraded++;
        return acc;
...
function buildDescriptionBadge(report, deltaSummary, deltas) {
    ...
    if (report && report.summary.totalViolations > 0) {
        ...
        const byMetric = (0, collect_js_1.default)(Object.values(report.files))
        ...
        const metricOrder = ['cyclomatic', 'cognitive', 'halstead_effort', 'halstead_bugs'];
        const rows = (0, collect_js_1.default)(metricOrder)
...
function formatHalsteadContext(violation) {
    if (!violation.metricType?.startsWith('halstead_'))
        return '';
