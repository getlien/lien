...
type ReviewStyle = 'line' | 'summary' | 'hybrid';
  ...
  try {
    ...
    if (config.reviewStyle === 'summary') {
      ...
      await postLineReview(
        ...
        codeSnippets,
        config,
        false // don't limit to errors only
      );
    } else {
      // hybrid mode: inline for errors, summary for warnings
      await postHybridReview(
        octokit,
        prContext,
        report,
        sortedViolations,
 ...
 */
async function postHybridReview(
  octokit: ReturnType<typeof createOctokit>,
  prContext: ReturnType<typeof getPRContext> & object,
  report: Awaited<ReturnType<typeof runComplexityAnalysis>> & object,
  violations: ComplexityViolation[],
