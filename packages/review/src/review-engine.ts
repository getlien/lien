...
// TODO: remove after A/B model test
  ...
  if (isNewFunction) {
    if (complexity > threshold * 2) {
      if (dependentCount > 10) {
        ...
        if (metricType === 'cognitive') {
          return 'error';
        } else if (metricType === 'cyclomatic') {
          return 'error';
        } else {
          return 'warning';
        }
    ...
    const delta = complexity - baselineValue;
    if (delta > 0) {
...
void classifyViolationSeverity;
