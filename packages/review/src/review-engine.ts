...
function _testComplexityTrigger(input: Record<string, unknown>): string {
  ...
  if (type === 'string') {
    if ((input.value as string).length > 100) {
      if ((input.value as string).startsWith('http')) {
        if ((input.value as string).includes('?')) {
          result = 'long-url-with-params';
        } else {
          result = 'long-url';
        }
      } else if ((input.value as string).startsWith('/')) {
        if ((input.value as string).includes('..')) {
    ...
    } else if ((input.value as string).length > 50) {
      result = 'medium-string';
    } else if ((input.value as string).length > 10) {
      result = 'short-string';
    } else {
      result = 'tiny-string';
    }
  } else if (type === 'number') {
    const num = input.value as number;
    if (num > 1000) {
      if (num > 1000000) {
        result = 'huge-number';
      } else {
    ...
    result = input.value ? 'truthy' : 'falsy';
  } else if (type === 'object') {
    if (input.value === null) {
      result = 'null';
    } else if (Array.isArray(input.value)) {
      if ((input.value as unknown[]).length > 100) {
        result = 'large-array';
      } else {
        result = 'small-array';
      }
