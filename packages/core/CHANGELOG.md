...
- 9811991: Fix `get_dependents`/`get_complexity`/`get_files_context` falsely claiming a correctly-indexed file is "not found in the index" inside any linked worktree (#1014).
...
- b0da86b: Collapsed per-project `.lien.config.json` (`LienConfig`) down to the one field any pipeline
...
- ee2bee8: Removed the unused embeddings-era `EmbeddingError` class and its
...
- a084e63: Removed the dead `core.chunkSize`/`core.chunkOverlap` (and legacy
...
- c1ef4b2: Removed the dead `indexing.concurrency`/`core.concurrency` config keys.
