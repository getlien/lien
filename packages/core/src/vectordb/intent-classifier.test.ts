...
describe('Query Intent Classification', () => {
  describe('LOCATION Intent', () => {
    ...
    it('should detect "where can I find" queries', () => {
      expect(classifyQueryIntent('where can i find the database models')).toBe(
  ...
  describe('CONCEPTUAL Intent', () => {
    ...
    it('should detect "what is" queries', () => {
      expect(classifyQueryIntent('what is the authentication service')).toBe(
    ...
    it('should detect "understand" queries', () => {
      ...
      expect(classifyQueryIntent('I want to understand the cache layer')).toBe(
        QueryIntent.CONCEPTUAL,
      );
    });
  ...
  describe('IMPLEMENTATION Intent', () => {
    it('should detect "how is X implemented" queries', () => {
      expect(classifyQueryIntent('how is authentication implemented')).toBe(
    ...
    it('should detect "how are X implemented" queries', () => {
      expect(classifyQueryIntent('how are the routes implemented')).toBe(
        QueryIntent.IMPLEMENTATION,
      );
      expect(classifyQueryIntent('How are permissions implemented')).toBe(
        QueryIntent.IMPLEMENTATION,
      );
    });
    ...
    it('should detect "implementation of" queries', () => {
      expect(classifyQueryIntent('implementation of user authentication')).toBe(
        QueryIntent.IMPLEMENTATION,
      );
      expect(classifyQueryIntent('Implementation of the cache layer')).toBe(
        QueryIntent.IMPLEMENTATION,
      );
    });

    it('should detect "source code for" queries', () => {
      expect(classifyQueryIntent('source code for authentication')).toBe(
        QueryIntent.IMPLEMENTATION,
      );
      expect(classifyQueryIntent('Source code for the API handler')).toBe(
        QueryIntent.IMPLEMENTATION,
      );
    });

    it('should handle queries with extra whitespace', () => {
      expect(classifyQueryIntent('  how  is  auth  implemented  ')).toBe(
        QueryIntent.IMPLEMENTATION,
      );
    });
  ...
  describe('Real-World Queries from Dogfooding', () => {
    it('should correctly classify dogfooding analysis queries', () => {
      ...
      expect(classifyQueryIntent('How does the indexing process work from start to finish?')).toBe(
        QueryIntent.CONCEPTUAL,
      );
      expect(classifyQueryIntent('Where is the main indexing logic located?')).toBe(
        QueryIntent.LOCATION,
      );
      expect(classifyQueryIntent('How is the MCP server implemented?')).toBe(
        QueryIntent.IMPLEMENTATION,
      );
      ...
      expect(classifyQueryIntent('Configuration migration system implementation')).toBe(
        QueryIntent.IMPLEMENTATION,
      );
    });
  });
  ...
  describe('Helper Functions', () => {
    ...
    describe('addIntentRule', () => {
      it('should allow adding custom rules with cleanup', async () => {
        const { addIntentRule, classifyQueryIntent, QueryIntent } =
          await import('./intent-classifier.js');
        ...
        expect(classifyQueryIntent('this matches custom test pattern')).toBe(
          QueryIntent.IMPLEMENTATION,
        );
      });
    ...
    describe('resetIntentRules', () => {
      it('should reset to original rules', async () => {
        const { addIntentRule, resetIntentRules, getIntentRules, QueryIntent } =
          await import('./intent-classifier.js');
