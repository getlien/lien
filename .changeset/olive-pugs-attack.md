...
- **Files above 5 MB are skipped**, matching the cap `lien index` has always applied. An 8 MB source file cost roughly a gigabyte of memory to chunk, then exceeded the native parser's string limit, fell back to line-based chunking, and landed in the report carrying meaningless complexity metrics.
