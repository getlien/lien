...
It reads the working tree directly, with no persisted index, and always exits 0 — `lien complexity --fail-on` and `lien delta` remain the gates. Flags: `--top`, `--path`, `--include-tests`, `--format text|json`.
