...
- **Cache/Sessions/Queues:** Valkey 8 (Redis-compatible, uses `predis` client)
- **Auth:** GitHub OAuth via Laravel Socialite
- **Dev environment:** OrbStack (local Kubernetes)

## PRD Reference

The product requirements document lives at `.wip/prd.md` (symlinked from the lien repo). Read it before implementing any app-specific features.

## Style Guide

**Read `STYLE_GUIDE.md` before making any UI or frontend changes.** It defines the design identity: typography (Geist Sans/Mono), color system (dark-first, zinc neutrals, teal accent), surface hierarchy, component patterns, motion, and accessibility requirements. All frontend work must conform to the style guide.
...
- Place DTOs in `app/DataTransferObjects/` alongside the domain they serve

---

## Naming Conventions

- **PHP Classes:** PascalCase (`ReviewService`, `PullRequestController`)
- **PHP Methods/Variables:** camelCase (`getReviewStatus`, `$pullRequest`)
...
```bash
./vendor/bin/pint --test    # Code style must pass
...
**Tip:** Run `./vendor/bin/pint` (without `--test`) to auto-fix style issues.
...
```bash
./k8s/deploy.sh                                                  # Build & deploy everything
kubectl get pods -n lien                                        # Check pod status
kubectl logs deployment/laravel -n lien                         # App logs
kubectl logs deployment/laravel-worker -n lien                  # Worker logs
kubectl exec -it deployment/laravel -n lien -- php artisan tinker   # Tinker
kubectl exec deployment/laravel -n lien -- php artisan test     # Run tests
kubectl delete namespace lien                                   # Teardown everything
```
