...
A release/validation pass (changesets, the existing post-publish registry smoke test, and a fresh install on a stock Apple toolchain) follows the flip before general availability. Implementation work landed on `feat/parser-native` starting at Phase 1.
...
Six non-blocking issues surfaced by adversarial verification and tracked into later phases (one, parse-stage concurrency, was already resolved shortly after, see below):
