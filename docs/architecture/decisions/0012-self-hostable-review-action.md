...
- Same-day hardening: collapsing two separate Lien checks into one (#584), defaulting the check to advisory (`fail-on: never`, #585), and later pinning the agent-review model default (#592, `moonshotai/kimi-k2.7-code`).
  **Update (2026-08-03):** the advisory default is still what every other `@liendev/review` / `@liendev/action` consumer gets, but this monorepo's own `.github/workflows/lien-review.yml` now opts out of it, setting `fail-on: error`. Motivated by #1068: three live defects (two fabrication bugs) merged past a green `review` check in one day, each caught only by findings the check reported as success — the severity gate this ADR shipped as advisory-by-default had never been wired up for this repo's own PRs.
  **Update (2026-08-31):** that workflow is deleted; this repo no longer reviews its own PRs with Lien Review, and CodeRabbit is the PR review. The trigger was #1125, a pure file move of 9,226 LOC, where `fail-on: error` fired on two complexity violations in a function whose diff was two imports and two type annotations — the review engine counts a moved file as new, so every pre-existing violation in it reads as newly introduced, while `lien delta --base` correctly passed. Two runs of that review also died on OpenRouter's in-flight credit cap. Paying per PR to be told a rename introduced complexity it did not stopped being worth it, given the engine is being cut anyway (see the CLI simplification plan). `packages/action` and the consumer-facing distribution below are untouched by this and still exist.
