...
Status: Shipped (0.55.0; concurrency hardening landed in 0.59.0)
...
`git worktree list --porcelain`, which is authoritative and also correct for
...
- **Overlay stamped with base's version.** On serve start (overlay mode) compare
  ...
  - **moved** (main was reindexed) → the base hashes the overlay was diffed
    ...
    chunking only the still-diverged files, which is cheap.
...
**Overlay file size (clear-and-rebuild).** `OverlayBackend.clear()` originally
used an in-place `DELETE FROM chunks` (and mask/meta tables), which leaves
freed pages in SQLite's freelist instead of shrinking the file. Since
`buildOverlay` calls `clear()` at the start of every rebuild, an overlay that
once held many diverged files kept that high-water-mark file size forever even
after shrinking back down to a handful of live rows; on the real repo this
made a "small" overlay 11 MB, nearly the size of the 11.5 MB base.

The first fix attempt mirrored `SqliteBackend.clear()`'s pattern (close the
handle, delete the db + WAL/SHM files, reopen fresh): it did shrink the file
(11 MB → 1.0 MB) but proved unsafe under concurrency: it swaps the overlay
file's identity out from under any other process that has it open, and a
concurrent `lien index` stress test reproduced `disk I/O error` failures in
roughly half the runs. The shipped fix instead does `DELETE` + `VACUUM` + a WAL
checkpoint: same file identity throughout, same disk-space reclamation
(confirmed 1.1 MB on the real repo), zero failures under the same stress test.
See `packages/core/src/vectordb/overlay-backend.ts` and its disk-space
regression test.

One documented-but-not-fixed edge case remains: transient state mixing when
...
The DELETE+VACUUM fix above still left `buildOverlay` mutating the mask and
chunks across many autocommitted statements spread over async ticks, and
`recordBaseBuild` bumped `.lien-index-version` on every rebuild including
no-op ones. Dogfooding surfaced both: a worktree serve's `list_functions` /
`querySymbols` intermittently returned 0 results for a class that exists, and
the overlay's `indexVersion` churned with zero file edits while more than one
serve process had the worktree as cwd (multiple serves per root is a known
operational reality; they pile up across install paths).

Two composing root causes, both now fixed:

### 1. Non-atomic rebuild could expose mask-without-replacement

Between `clear()` and the final `insertBatch`, an added file had no overlay
rows and no base fallback for the entire scan/chunk phase; between a modified
file's `deleteByFile` (which sets the mask) and its `insertBatch`, the file
was masked with no rows. A concurrent reader on another connection (another
serve) could observe those states.

**Fix (reader-atomic swap).** All async work (scan, hash, chunk) now happens
in `buildOverlay` *before* any write, producing in-memory chunk batches plus
the mask set. `OverlayBackend.applyRebuild` then applies the entire swap
(delete old chunks + mask, insert new chunks + mask, update meta) in one
...
state. `buildOverlay` no longer calls `clear()` directly; the transaction's
`DELETE` reclaims logically, and a post-commit `VACUUM` +
`wal_checkpoint(TRUNCATE)` (VACUUM cannot run inside a transaction) reclaims
on disk, preserving the same-file-identity fix above.

The reader side is snapshotted too: each union read (`unionRecords`, `search`,
`scanPaginated`) pairs its chunk scan with its mask read inside one deferred
read transaction, pinning a single WAL snapshot. Otherwise an atomic writer
commit landing between the reader's two statements would still be observed
half-applied.
...
transactions are per-connection, so no code structure can extend `S` across
the base: true cross-database snapshot atomicity is impossible in the
...
produce the mask-without-replacement bug or duplicates: base hits are filtered
against the mask from the same snapshot `S` as the overlay hits, so any file
with overlay rows that also exists in base is masked in `S` regardless of when
the base read runs, including across a concurrent `applyRebuild` commit. The
only residual effect is that base rows may come from a base commit that
landed mid-query, with each side internally consistent and per-file single-source
preserved. But a mid-serve base write already implies the overlay is
stale-until-rebuild regardless of intra-query timing (see "Staleness &
revalidation"). Cross-corpus BM25 ranking is likewise already approximate by
design (see "FTS caveat (v1)").

### 2. Rebuild livelock from version-stamp churn

`recordBaseBuild` bumped `.lien-index-version` on every rebuild. The only
overlay-rebuild trigger is each serve's startup `handleAutoIndexing` (tool
handlers and the version/git polls only reconnect, never rebuild). Every
startup rebuild bumped the stamp; every bump made all other serves' version
poll reconnect; restart/re-spawn churn kept new serves rebuilding: a
self-amplifying reconnect/rebuild storm with no file edits.

**Fix (no-op means no bump).** `buildOverlay` computes a content signature: a
hash over an indexing-format salt (`INDEX_FORMAT_VERSION` plus chunk
size/overlap, so a Lien upgrade that changes the chunking contract forces one
real rebuild instead of skipping forever) plus the sorted (diverged file,
content hash) pairs plus the sorted mask set, reusing hashes already computed
during the diff. If it matches the signature stored at the last build, the
rebuild is a no-op: it refreshes only the base stamp (so `needsRebuild()`
still settles after a base reindex) and returns `changed: false` without
bumping. A genuine content change still bumps, so real edits propagate.
`applyRebuild` re-checks the signature inside its `IMMEDIATE` transaction, so
even under a true race the swap is serialized and only the first writer
bumps; a peer that already applied the identical overlay makes this a no-op.

### Cross-process single-rebuilder guard: what we chose

A dedicated single-rebuilder advisory lock (`BEGIN IMMEDIATE` held for the
whole build, with busy-skip, "its result serves both") was considered and
rejected in favor of:

- the in-transaction signature test-and-set under `BEGIN IMMEDIATE`, which
  already guarantees a single effective rebuild (one bump, silent no-ops)
  across any number of concurrent writers, and
...
A held advisory lock (or a lease row with a staleness timeout) would have to
span the async chunking phase, blocking the watcher's incremental writes for
the whole build, and needs fragile cross-async transaction handling plus
stale-lock/lease heuristics and their own tests. Its only remaining benefit
over the chosen design is de-duplicating concurrent chunking CPU, which is
negligible given the overlay covers only a handful of diverged files
(KISS/YAGNI). Multiple serves are therefore tolerated (atomic, single-bump,
busy-skip), not assumed away.

Covered by a reproduction-first stress test (`overlay-concurrency.test.ts`):
two `OverlayBackend` connections on one overlay db, a writer rebuilding in a
loop while a reader polls `querySymbols` for diverged files, plus direct
assertions that a no-op doesn't bump the version stamp and that a genuine
change still does. See `packages/core/src/vectordb/overlay-backend.ts`
(`applyRebuild`, `overlaySnapshot`, `refreshBaseStamp`),
`packages/core/src/indexer/overlay-index.ts` (`buildOverlay`,
`computeOverlaySignature`), and
