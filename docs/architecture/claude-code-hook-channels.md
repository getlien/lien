...
`augment-explore-task.sh` fires normally. This was confirmed by replaying
both `tool_name: "Agent"` and `tool_name: "Task"` payloads through the
script, and is now covered permanently by
`packages/cli/test/augment-explore-hook.test.ts`.
