...
anymore — the Rust side owns the actual grammar; the TS side only ever sees
`SyntaxNode`/`Tree` reconstructed from the wire format.
...
Merging **that** publishes to npm — irreversible, leave it for the
maintainer.
