...
2. **Check per-vote cost/turns before reading a red batch as behavioral.**
   ...
   pass never ran fails the check loudly with the cause named; harness
   batches still need the per-vote autopsy.)
3. **Real billing ≈ 1.5-2x harness-reported cost**, and the repo's CI runs a
...
5. **Prefer zero-LLM fixes.** Assertion changes can be validated for free by
   ...
   (system prompt + inlined tool results + "emit your verdict now"): about
   $0.03 per sample. Only genuine behavior changes need fresh calibration.
...
`AgentReviewPlugin.analyze()` can run a dedicated, single-rule pass for a
rule whose candidates are enumerable and whose verdict set is closed,
instead of asking it to compete for space in the shared findings list.
Doc-truth was the original case (PR #733) and proved the mechanism.
[Agent-Review Pass Architecture](../architecture/review-pass-architecture.md)
and [ADR-014](../architecture/decisions/0014-per-rule-candidate-loop-passes.md)
own the `ReviewPassSpec` contract that generalized it, which passes exist
today, and each one's current on/off status: read both before touching a
pass or deciding whether a new rule needs one.

The judgment worth keeping here: reach for a dedicated pass only after
proving the shared findings list, not the prompt, is the bottleneck (see
the deterministic-signal section above), and only for a rule whose
candidates form a closed, enumerable set. An open-investigation rule like
`concurrency-race` or `boundary-change` does not fit that shape.
...
- The shipped candidate-loop passes: `doc-truth-pass.ts`,
  ...
  `removed-exports-pass.ts`, `docs-drift-pass.ts` (same directory)
