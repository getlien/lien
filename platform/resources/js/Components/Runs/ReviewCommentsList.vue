  ...
  return (
    {
      ...
      architectural: 'bg-cyan-900/30 text-cyan-400',
      bugs: 'bg-red-900/30 text-red-400',
...
function typeDotColor(type) {
  return (
    {
      complexity: 'bg-amber-400',
      architectural: 'bg-cyan-400',
      bugs: 'bg-red-400',
      summary: 'bg-brand-400',
    }[type] || 'bg-zinc-400'
  );
}
...
function typeIcon(type) {
  return (
    {
      bugs: 'M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z',
      architectural:
...
<template>
  <div>
          ...
          <div v-for="comment in fileComments" :key="comment.id" class="px-4 py-3">
              ...
              <div class="flex shrink-0 items-center gap-2">
                ...
                <span
                  :class="[
                    ...
                    typeColor(comment.review_type),
                  ]"
                >
                  <span
                    :class="[
                      'inline-block h-1.5 w-1.5 rounded-full',
                      typeDotColor(comment.review_type),
                    ]"
                  />
                  {{ comment.review_type }}
                </span>
              ...
              <div class="ml-auto flex shrink-0 items-center gap-2">
                <span
                  ...
                  :class="[
                    ...
                    resolutionColor(comment.resolution),
                  ]"
                >
            ...
            <div class="mt-2">
              <p
                ...
                :class="{
                  'line-clamp-3': isLong(comment.body) && !expandedComments.has(comment.id),
                }"
              >
                {{ comment.body }}
              </p>
    ...
    <div
      v-if="filteredComments.length === 0 && comments.length > 0"
      class="mt-3 rounded-lg border border-zinc-800 bg-zinc-900 p-8 text-center"
    >
