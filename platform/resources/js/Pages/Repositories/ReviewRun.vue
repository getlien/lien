...
const isTerminal = computed(() => ['completed', 'failed'].includes(props.reviewRun.status));
    ...
    <div>
      ...
      <Breadcrumbs
        :items="[
          ...
          { label: 'Runs', href: `/repos/${repository.id}/runs` },
          { label: reviewRun.pr_number ? `PR #${reviewRun.pr_number}` : `Run #${reviewRun.id}` },
        ]"
        class="mb-4"
      />
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-medium text-zinc-100">
            ...
            <template v-else> Baseline Analysis </template>
          </h1>
          <div class="mt-1 space-y-0.5">
            <p class="text-sm text-zinc-400">
              <template v-if="reviewRun.head_ref">
                ...
                <CopyableCode
                  :text="reviewRun.head_sha"
                  :display="shortSha(reviewRun.head_sha)"
                  class="ml-1"
                />
              </template>
              <template v-else>
              ...
              <template v-if="reviewRun.base_sha">
                ...
                <template v-if="reviewRun.base_ref">
                  ...
                  <CopyableCode
                    :text="reviewRun.base_sha"
                    :display="shortSha(reviewRun.base_sha)"
                    class="ml-1"
                  />
                </template>
                <template v-else>
          ...
          <div v-if="complexitySnapshots?.length" class="deferred-enter mt-8">
            <h2 class="text-lg font-medium text-zinc-100">
              ...
              <span class="text-sm font-normal text-zinc-400"
                >({{ complexitySnapshots.length }} functions)</span
              >
            </h2>
            <div class="mt-3">
      ...
      <div
        v-else-if="reviewRun.status === 'failed'"
        class="mt-8 rounded-lg border border-red-500/20 bg-red-900/30 p-5"
      >
