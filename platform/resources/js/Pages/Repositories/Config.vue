  ...
  <AuthenticatedLayout>
    <div class="mx-auto max-w-3xl">
      ...
      <div
        ...
        class="mt-4 rounded-lg border border-red-500/20 bg-red-900/30 p-4"
      >
        <p v-for="(error, key) in form.errors" :key="key" class="text-sm text-red-400">
          {{ error }}
        </p>
      </div>
      ...
      <form class="mt-8 space-y-8" @submit.prevent="submit">
          ...
          <div class="mt-4 space-y-4">
            <div class="flex items-center justify-between">
              ...
              <button
                ...
                @click="
                  form.review_config.complexity.enabled = !form.review_config.complexity.enabled
                "
              >
                <span
            ...
            <div>
              ...
              />
              <p
                v-if="form.errors['review_config.complexity.threshold']"
        ...
        <div class="rounded-lg border border-zinc-800 bg-zinc-900 p-5">
          ...
          <p class="mt-1 text-sm text-zinc-400">
            Post a risk-assessed summary to the PR description.
          </p>
        ...
        <div class="rounded-lg border border-zinc-800 bg-zinc-900 p-5">
          <h2 class="text-lg font-medium text-zinc-100">Bug Finder</h2>
          <p class="mt-1 text-sm text-zinc-400">
            Analyzes changed functions and their callers to detect bugs. Team+ plans.
          </p>

          <div class="mt-4 flex items-center justify-between">
            <div>
              <label for="bugs-enabled" class="font-medium text-zinc-200">Enabled</label>
              <span class="ml-2 rounded bg-zinc-800 px-2 py-0.5 text-xs text-zinc-400">
                {{ configSource('bugs.enabled') }}
              </span>
            </div>
            <button
              id="bugs-enabled"
              type="button"
              role="switch"
              aria-label="Enable bug finder"
              :aria-checked="form.review_config.bugs.enabled"
              class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors"
              :class="form.review_config.bugs.enabled ? 'bg-brand-500' : 'bg-zinc-700'"
              @click="form.review_config.bugs.enabled = !form.review_config.bugs.enabled"
            >
              <span
                class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transition-transform"
                :class="form.review_config.bugs.enabled ? 'translate-x-5' : 'translate-x-0'"
              />
            </button>
          </div>
        </div>
